...
func parseYoutubeURL(parsed *url.URL) (model.Type, string, error) {
	...
	// - https://www.youtube.com/c/CustomName/videos
	// Treat custom URLs like handles - youtube-dl will resolve them
	if strings.HasPrefix(path, "/c/") {
		kind := model.TypeHandle

		parts := strings.Split(parsed.EscapedPath(), "/")
		if len(parts) <= 2 {
			return "", "", errors.New("invalid custom URL link")
		}

		customName := parts[2]
		if customName == "" {
			return "", "", errors.New("invalid custom name")
		}

		// Use the custom name as the ID - youtube-dl will resolve it
		return kind, customName, nil
	}
